...
By reusing Karate test we can extend a performance testing framework usiing Gatling (and Scala) to defining a load-model. In this project a sample simulation file(not executable) can be found under classpath:trafficAPI/UserSimulation , which does the user injection to already defined test scenarios. Note that due to some version conflicts the stress tests are not executable right now.
